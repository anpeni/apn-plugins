...
```diff
  ...
  backend.add(import('@ddc-neoris/plugin-jenkins-backend'));
  backend.start();
```
...
yarn --cwd packages/backend add @ddc-neoris/plugin-jenkins-backend
```
...
} from '@ddc-neoris/plugin-jenkins-backend';
